...
namespace TriadRNSim.Forms
{
    public partial class frmRoutines : Form
    {
        ...
        private void lstRoutines_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            btnDelete.Enabled = btnChange.Enabled = item != null && !internalObjects.ContainsKey(item);
        }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (frm.Successed)
            {
                StreamWriter fileOutput = new StreamWriter(frm.ResultRoutine.Name + ".txt", false, Encoding.Default);
                fileOutput.WriteLine(frm.ResultRoutine.Text);
                fileOutput.Close();
                ...
                lstRoutines.Items.Add(item);
                frmMain.ontologyManager = new COWLOntologyManager(frmMain.sOntologyPath);
            }
        }
        ...
        private void btnChange_Click(object sender, EventArgs e)
        {
            ...
            frm.OnNameChecked += delegate (object s, CancelEventArgs args)
            {
                foreach (ListViewItem items in lstRoutines.Items)
                {
                    if (items.Index != item.Index && items.Text.Equals(frm.DesignTypeName, StringComparison.CurrentCultureIgnoreCase))
                    {
                        args.Cancel = true;
                        break;
                    }
            ...
            {
                MessageBox.Show("Не удалось открыть рутину :( \nОтсутствует текстовое представление!", "Ошибка!");
                return;
            }
            StreamReader fileInput = new StreamReader(fileName, Encoding.Default);
            frm.Code = fileInput.ReadToEnd();
            fileInput.Close();
