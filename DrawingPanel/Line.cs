...
namespace DrawingPanel
{
    ...
    public class Line : BaseObject
    {
        ...
        {
            if (FromCP.Owner == ToCP.Owner)
            {
                gp.AddArc((this.getX() + dx-R) * zoom, (this.getY() + dy-R) * zoom, R * zoom, R * zoom, 90, 270);
            }
        ...
        public override void Draw(Graphics g, int dx, int dy, float zoom)
        {
            ...
            if (FromCP.Owner == ToCP.Owner) //если точка исходящая равна входящей, то рисуем петлю
            {
                g.DrawArc(myPen, (this.X + dx - R + 2) * zoom, (this.Y + dy-R + 2) * zoom, R * zoom, R * zoom, 90, 270);
